...
class PPO(object):
    ...
    def __init__(self, sess, S_DIM, A_DIM, A_LR, C_LR, A_UPDATE_STEPS, C_UPDATE_STEPS, METHOD):
        ...
        with tf.variable_scope('critic'):
            ...
            #self.ctrain_op = tf.train.GradientDescentOptimizer(self.C_LR).minimize(self.closs)
...
#            with tf.variable_scope('surrogate'):
            self.ratio = tf.exp(self.pi.log_prob(self.tfa) - self.oldpi.log_prob(self.tfa))
                ...
                # ratio is of size 3 (A_DIM); reduce it to size 1
...
#               surr = self.ratio * self.tfadv
        ...
        with tf.variable_scope('atrain'):
            ...
            #self.atrain_op = tf.train.GradientDescentOptimizer(self.A_LR).minimize(self.aloss)        
...
#                    break
    ...
    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            ...
            #mu_st = tf.scalar_mul(0.5,mu_st) # scalar mult
            #mu_acc = tf.scalar_mul(0.5,mu_acc) # scalar mult
